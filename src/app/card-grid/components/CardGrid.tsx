...
const GridInner = observer(function GridInner() {
  ...
  return (
    <div className="flex flex-col gap-6 pb-20 h-full min-h-[100svh]">
      <section
        ref={scrollRef}
        className="min-h-0 flex-1 overflow-y-auto grid gap-2 sm:gap-3 grid-cols-[repeat(auto-fit,minmax(160px,1fr))]"
      >
        {cards.map((_, i) => (
          <div key={i} data-card-index={i} className="flex items-center justify-center p-3">
